...
use crate::{
  views::{
    inputs::{add_input_view,get_input_view,validate,update_input_view,delete_input_view}
  },
  utils::{
    StateCustom,
    ReturnError,
...
#[put("/<id>",data="<data>")]
pub async fn update_input<'a>(id:&str,data:Json<Input>,client:&State<StateCustom>) -> Result<Json<ReturnMessage<'a>>,Json<ReturnErrors>>{
  let errors = validate(&data.0).await;
  if errors.errors.len() > 0{
    Err(Json(errors))
  }else{
    update_input_view(id, data.0, &client.client).await
  }
}
